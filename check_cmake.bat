...
if %cmake_ver_minmaj% LSS %cmake_minversion_minmaj% (
  echo(
  echo FURo-Sim requires cmake version %cmake_minversion_minmaj% but you have %cmake_ver_minmaj% which is older.  >&2
  goto :download_install
)
...
:download_install
echo cmake ^>= 3.14 is required. Install it from https://cmake.org/download (or the Visual Studio "C++ CMake tools for Windows" component) and make sure it is on PATH.
exit /b 1
